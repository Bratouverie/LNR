...
// ═══════════════════════════════════════════════════════════════
// getAssemblyPoints — Lists assembly points (super_admin only)
//
// Body: { token: string }
// Returns: { assemblyPoints: [...] }
// ═══════════════════════════════════════════════════════════════
...
Deno.serve(async (req) => {
  try {
    ...
    const assemblyPoints = await base44.asServiceRole.entities.AssemblyPoint.list('-created_date', 200);

    return Response.json({ assemblyPoints });
  } catch (error) {
