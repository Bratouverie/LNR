  ...
  try {
    ...
    if (manager.role !== 'super_admin') {
      await base44.asServiceRole.entities.AuditLog.create({
        ...
        action: 'review_delete_denied',
        resource: `review:${reviewId}`,
        statusCode: 403,
        details: { reason: 'insufficient_role', role: manager.role },
      });
      return Response.json({ error: 'Only super_admin can delete reviews' }, { status: 403 });
    }
    ...
    await base44.asServiceRole.entities.AuditLog.create({
      ...
      statusCode: 200,
      ipAddress: ipAddr,
      details: { reviewName: updated?.name || null },
    });
