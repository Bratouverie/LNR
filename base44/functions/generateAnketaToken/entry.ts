...
// ═══════════════════════════════════════════════════════════════
// generateAnketaToken — Creates a 7-day JWT magic link for candidate anketa
//
// Body: { token: string (manager JWT), candidateId: string }
// Returns: { success, anketaToken, expiresAt, link }
// ═══════════════════════════════════════════════════════════════
...
const ANKETA_TOKEN_TTL_SEC = 7 * 24 * 60 * 60; // 7 days
...
Deno.serve(async (req) => {
  try {
    ...
    // ── 3. Generate anketa JWT (7 day expiry) ──
    const exp = Math.floor(Date.now() / 1000) + ANKETA_TOKEN_TTL_SEC;
    const anketaJwt = await generateJWT(
      { candidateId, type: 'anketa', exp },
      Deno.env.get('JWT_SECRET')
    );

    // ── 4. Invalidate previous unused tokens for this candidate ──
    const oldTokens = await base44.asServiceRole.entities.AnketaToken.filter({
      candidateId,
      usedAt: null,
    });
    for (const old of oldTokens) {
      await base44.asServiceRole.entities.AnketaToken.update(old.id, { usedAt: new Date().toISOString() });
    }

    // ── 5. Create new AnketaToken ──
    const expiresAt = new Date(exp * 1000).toISOString();
    const anketaToken = await base44.asServiceRole.entities.AnketaToken.create({
      candidateId,
      token: anketaJwt,
      expiresAt,
    });
    ...
    await base44.asServiceRole.entities.CandidateLog.create({
      ...
      reason: 'anketa_link_generated',
    });

    await base44.asServiceRole.entities.AuditLog.create({
      actor: `manager:${manager.id}`,
      action: 'anketa_token_generated',
